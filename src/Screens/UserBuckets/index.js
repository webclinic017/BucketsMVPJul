...
const Portfolio = (props) => {
  ...
  return (
    <>
      ...
      <div className="p-11 flex flex-col min-h-screen commonBackGround">
        {/* <div className="flex justify-between">
          <div className="flex items-center">
            ...
            />
            <div>
              <h3 className="font-bold text-gray-400 text-4xl">
            ...
            {!isFetchingBuckets && metrics && (
              <>
                <div>
                  ...
                  >
                    {metrics.valueReturn > 0 && "+"}
                    {metrics.valueReturn.toFixed(2)}
                  </span>
                  <span className="text-lg text-gray-400 mx-1">|</span>
                  <span
                    style={{
                      color:
                        metrics.percentReturn > 0
                          ? theme.colors.green
                          : theme.colors.red,
                    }}
                    className="font-bold text-md"
                  >
                    {metrics.percentReturn > 0 && "+"}
                    {metrics.percentReturn.toFixed(2)}%
                  </span>
                </div>
                <span
          ...
          <div className="block sm:block md:flex justify-between mt-6">
            <div className="w-full sm:w-full md:w-full lg:w-2/5">
              {buckets.map((bucket, index) => (
                <div
                  key={index}
                  onClick={() => handleOnClickBucket(bucket)}
                  className="flex mt-4 min-w-full items-center cursor-pointer justify-between"
                >
                  <div className="flex items-center">
                    <img
                      className="mx-2 rounded-full w-12 h-12 object-contain"
                      src={
                        bucket.userId === user._id
                          ? user.profilePicture
                          : BucketsLogo
                      }
                    />
                    <div>
                      ...
                      {bucket.hasMetrics && (
                        <div>
                          <span className="font-bold text-md text-gray-500">
                            ${bucket.currentValue.toFixed(2)}
                          </span>
                          <span className="text-lg text-gray-400 mx-1">|</span>
                          <span
                            style={{
                              color:
                                bucket.valueReturn > 0
                                  ? theme.colors.green
                                  : theme.colors.red,
                            }}
                            className="font-bold text-md"
                          >
                            {bucket.valueReturn > 0 && "+"}
                            {bucket.valueReturn.toFixed(2)}
                          </span>
                          <span className="text-lg text-gray-400 mx-1">|</span>
                          <span
                            style={{
                              color:
                                bucket.percentReturn > 0
                                  ? theme.colors.green
                                  : theme.colors.red,
                            }}
                            className="font-bold text-md"
                          >
                            {bucket.percentReturn > 0 && "+"}
                            {bucket.percentReturn.toFixed(2)}%
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <img
                    className="mx-6 w-4 h-4 object-contain"
                    src={ForwardArrow}
                  />
                </div>
              ))}
              ...
              <Link to="/home">
                <div className="m-4 relative flex flex-col items-center p-6 bg-tuscany-400 rounded shadow-xl">
                  ...
                  <h4 className="text-xl  font-bold leading-snug tracking-tight mb-1">
                    Browse Pre-Made Buckets
                  </h4>
                  <p className="text-gray-700 text-center">
                    The one stop shop for diversification and new ideas
                  </p>
                </div>
              </Link>
            </SmoothList>
