...
const Portfolio = (props) => {
  ...
  const isFetchingBucket = useSelector(
    (state) => state.bucket.isFetchingBucketData
  );
  const isFetchingBucketValue = useSelector(
    (state) => state.bucket.isFetchingBucketValue
  );
  ...
  const bucketHistoricalPrices = useSelector(
    (state) => state.bucket.bucketHistoricalPrices
  );
  const isFetchingBucketHistoricalPrices = useSelector(
    (state) => state.bucket.isFetchingBucketHistoricalPrices
  );

  useEffect(() => {
    dispatch(
      getBucketData({ bucketId }, (bucketValue) => {
        if (bucketValue > 0) {
  ...
  useEffect(() => {
    if (Object.keys(bucketData) && !isFetchingBucket) {
      ...
      if (bucketData.hasOwnProperty("stocks") && bucketData.stocks.length) {
        ...
        setBucketCostBasis(
          bucketData.stocks.reduce((total, stock) => stock.costBasis + total, 0)
        );
        ...
        bucketData.stocks.forEach((oStock) => {
          const fStock = stocksData.filter(
            (iStock) => oStock.ticker === iStock.symbol
          )[0];
          if (fStock.beta !== null) {
            beta += (oStock.targetWeight / 100) * parseFloat(fStock.beta);
          }
          if (
            fStock.size_type !== null &&
            fStock.size_type.toLowerCase() === "small"
          ) {
            size = { ...size, small: size.small + oStock.targetWeight };
          } else if (
            fStock.size_type !== null &&
            fStock.size_type.toLowerCase() === "medium"
          ) {
            size = { ...size, medium: size.medium + oStock.targetWeight };
          } else if (
            fStock.size_type !== null &&
            fStock.size_type.toLowerCase() === "large"
          ) {
          ...
          if (fStock.sector_basic_materials) {
            if (sectorWeights.hasOwnProperty("Basic Materials")) {
              sectorWeights["Basic Materials"] +=
                oStock.targetWeight * parseFloat(fStock.sector_basic_materials);
            } else {
              sectorWeights["Basic Materials"] =
                oStock.targetWeight * parseFloat(fStock.sector_basic_materials);
            }
          ...
          if (fStock.sector_consumer_cyclical) {
            if (sectorWeights.hasOwnProperty("Consumer Cyclical")) {
              sectorWeights["Consumer Cyclical"] +=
                oStock.targetWeight *
                parseFloat(fStock.sector_consumer_cyclical);
            } else {
              sectorWeights["Consumer Cyclical"] =
                oStock.targetWeight *
                parseFloat(fStock.sector_consumer_cyclical);
          ...
          if (fStock.sector_financial_services) {
            if (sectorWeights.hasOwnProperty("Financial Services")) {
              sectorWeights["Financial Services"] +=
                oStock.targetWeight *
                parseFloat(fStock.sector_financial_services);
            } else {
              sectorWeights["Financial Services"] =
                oStock.targetWeight *
                parseFloat(fStock.sector_financial_services);
          ...
          if (fStock.sector_real_estate) {
            if (sectorWeights.hasOwnProperty("Real Estate")) {
              sectorWeights["Real Estate"] +=
                oStock.targetWeight * parseFloat(fStock.sector_real_estate);
            } else {
              sectorWeights["Real Estate"] =
                oStock.targetWeight * parseFloat(fStock.sector_real_estate);
            }
          ...
          if (fStock.sector_consumer_defensive) {
            if (sectorWeights.hasOwnProperty("Consumer Defensive")) {
              sectorWeights["Consumer Defensive"] +=
                oStock.targetWeight *
                parseFloat(fStock.sector_consumer_defensive);
            } else {
              sectorWeights["Consumer Defensive"] =
                oStock.targetWeight *
                parseFloat(fStock.sector_consumer_defensive);
          ...
          if (fStock.sector_healthcare) {
            if (sectorWeights.hasOwnProperty("Sector Healthcare")) {
              sectorWeights["Sector Healthcare"] +=
                oStock.targetWeight * parseFloat(fStock.sector_healthcare);
            } else {
              sectorWeights["Sector Healthcare"] =
                oStock.targetWeight * parseFloat(fStock.sector_healthcare);
            }
          ...
          if (fStock.sector_basic_materials) {
            if (sectorWeights.hasOwnProperty("Sector Utilities")) {
              sectorWeights["Sector Utilities"] +=
                oStock.targetWeight * parseFloat(fStock.sector_basic_materials);
            } else {
              sectorWeights["Sector Utilities"] =
                oStock.targetWeight * parseFloat(fStock.sector_basic_materials);
          ...
          if (fStock.sector_communication_services) {
            if (sectorWeights.hasOwnProperty("Communication Services")) {
              sectorWeights["Communication Services"] +=
                oStock.targetWeight *
                parseFloat(fStock.sector_communication_services);
            } else {
              sectorWeights["Communication Services"] =
                oStock.targetWeight *
                parseFloat(fStock.sector_communication_services);
          ...
          if (fStock.sector_energy) {
            if (sectorWeights.hasOwnProperty("Sector Energy")) {
              sectorWeights["Sector Energy"] +=
                oStock.targetWeight * parseFloat(fStock.sector_energy);
            } else {
              sectorWeights["Sector Energy"] =
                oStock.targetWeight * parseFloat(fStock.sector_energy);
            }
          ...
          if (fStock.sector_industrials) {
            if (sectorWeights.hasOwnProperty("Industrials")) {
              sectorWeights["Industrials"] +=
                oStock.targetWeight * parseFloat(fStock.sector_industrials);
            } else {
              sectorWeights["Industrials"] =
                oStock.targetWeight * parseFloat(fStock.sector_industrials);
            }
          ...
          if (fStock.sector_technology) {
            if (sectorWeights.hasOwnProperty("Technology")) {
              sectorWeights["Technology"] +=
                oStock.targetWeight * parseFloat(fStock.sector_technology);
            } else {
              sectorWeights["Technology"] =
                oStock.targetWeight * parseFloat(fStock.sector_technology);
            }
  ...
  useEffect(() => {
    if (
      ...
      bucketData.stocks.map((stock) => {
        ...
        stock.numberDummyShares = bucketHistoricalPrices[ticker]
          ? amountInvested /
            bucketHistoricalPrices[ticker][
              bucketHistoricalPrices[ticker].length - 1
            ].close
          : "FAILSAFE-VAULE";
      ...
      const res = Array.from(
        valueArray.reduce(
          (m, { date, value }) => m.set(date, (m.get(date) || 0) + value),
          new Map()
        ),
  ...
  return (
    <>
      <div className="landing ">
        <Header isDemo={true} className="commonBackGround" />
      </div>
      <div className="p-11 flex flex-col min-h-screen commonBackGround underHeader">
        {isFetchingBucket ? (
          ...
          <>
            <div className="flex justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                />
                <span
                  onClick={handleOnClickBucketShare}
                  ...
                  <>
                    <DropdownMenu
                      trigger={
                        <img
                          src={OptionsIcon}
                          className="mx-1 mt-1 object-contain cursor-pointer w-5 h-5"
                        />
                      }
                    >
                      <DropdownItemGroup>
                        <DropdownItem onMouseDown={handleOnClickEdit}>
                          Edit
                        </DropdownItem>
            ...
            <div className="block sm:block md:flex justify-between mt-6">
              <div className="w-full sm:w-full md:w-full lg:w-2/5 ">
                {stocks.map((stock, index) => (
                  <StaticStockRow rowIndex={index} stock={stock} />
                ))}
              </div>
              <Helmet>
                ...
                <meta
                  name="description"
                  content="Check out this stock portfolio I made!"
                />
              </Helmet>
              <div className="w-full sm:w-full md:w-full lg:w-2/5 justify-center mr-4">
                {!isFetchingBucketValue &&
                  bucketValue &&
                  user &&
                  user._id === bucketData.userId && (
                    <div className="flex justify-center mb-4">
                      <span className="font-bold text-lg text-gray-500">
                        ${bucketValue.toFixed(2)}
                      </span>
                      ...
                      <span
                        style={{
                          color:
                            (bucketValue - bucketCostBasis) / bucketCostBasis >
                            0
                              ? theme.colors.green
                              : theme.colors.red,
                        }}
                ...
                <div className="flex-auto justify-items-center m-auto">
                    ...
                    <table className="table-auto ">
                      <div className="flex">
                        ...
                        <tbody>
                          <tr>
                            <td>
                              <span className="font-semibold text-l text-gray-600">
                                Yield
                              </span>
                            </td>
                            <td>
                              <span className="font-semibold text-l text-gray-500">
                                {bucketYield()}
                              </span>
                            </td>
                          </tr>
                          ...
                          <tr>
                            <td>
                              <span className="font-semibold text-l text-gray-600 mr-4">
                                Management Expense
                              </span>
                            </td>
                            <td>
                  ...
                  <div>
                    {" "}
                    <h4 className="text-l mt-12 font-semibold text-gray-500 my-5">
                      Compared to the average ETF, you save{" "}
                      {"$" + getMESavings()} in management fees every year with
                      this bucket on a $10,000 investment 💰 😀{" "}
                    </h4>
                  </div>
                </div>
                <div className="block sm:block md:flex flex-auto mb-20 ">
                  {bucketSectorWeights && (
                    <PieChart
                      ...
                      data={Object.entries(bucketSectorWeights).map(
                        ([key, value], i) => ({
                          value,
                          tooltip: key,
                          color: colors[i],
                      ...
                      total={Object.values(bucketSectorWeights).reduce(
                  ...
                  {bucketBeta && <GuageChart beta={bucketBeta} />}
                </div>
              </div>
            </div>
          </>
